...
Attacklab.wmdBase = function(){
	...
	util.prompt = function(text, defaultInputText, makeLinkMarkdown){
		...
		var createDialog = function(){
			...
			/* If we could add the optional title this would be great
			inputTitle = doc.createElement("input");
			inputTitle.type = "text";
			inputTitle.value = defaultInputText;
			inputTitle.name="bullshit";
			style = inputTitle.style;
			style.display = "block";
			style.width = "80%";
			style.marginLeft = style.marginRight = "auto";
	...
	wmd.editor = function(previewRefreshCallback){
		...
		var makeSpritedButtonRow = function(){
			...
			if((wmd.wmd_env.buttons.search("link"))>0){
		...
		var setupEditor = function(){
			...
			if (inputBox.form) {
				...
				inputBox.form.onsubmit = function(){
					//Bullraider, Customization; commented the following line to prevent
					// the form to transmit html data when onubmit occurs; For Empl: **strong text** 
					//will be transfered to the server as <Strong>strong text</Strong>.Since our
					//requirement needs to have the WMD text to be stored in the database and
					//displayed back with conversion applied while displaying the comments.
					//convertToHtml();
					if (submitCallback) {
...
/* might not be necessary, You MUST VERIFY before removing
	Attacklab.wmd();
	Attacklab.wmdBase();
