...
class CommunityViewGroups extends CommunityView
{
	...
	public function create( $data )
	{
		...
		$lists['categoryid']	=   CCategoryHelper::getSelectList( 'groups', $cTree );
	...
	public function display( $data )
	{
		...
		// Added section to get last reply author if there is any to display at sidebar latest discussion
		if (! empty($discussions[0]->lastreplied))
		{
			$lastReplier = $discussionModel->getLastReplier($discussions[0]->id);
			// Add is_null check to avoid earlier wall post being removed but lastreplied is with valid date
			if (!is_null($lastReplier))
			{
				$discussions[0]->lastReplier = $lastReplier->post_by;
			}
		}
	...
	{               
		...
		$approvedMembers		= $this->_getApproved($members);
		$membersCount	= count($approvedMembers);
		...
		$userids = array();
		foreach($approvedMembers as $uid){ $userids[] = $uid->id; }
		...
		*/
		...
		$tmpl->set( 'members' 			, $approvedMembers );
	...
	public function mygroups()
	{
		...
		/**
		 * Modified By Adam on 14 July 2011
