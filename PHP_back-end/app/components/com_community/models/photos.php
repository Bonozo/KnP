...
implements CLimitsInterface
{
	 ...
	 * @param	int	$id	The album id that we want to retrieve photos from
	 */	 	
	public function getPhotos( $id , $limit = null , $limitstart = null )
	{
		$db		=& $this->getDBO();
...
// 				. 'or (a.permissions = 40 and a.creator = '. $db->Quote( $userId ) . ')) '
	...
	public function _getAlbums( $id , $type , $pagination = false , $doubleLimit = false, $limit="" )
	{
		$db			=& $this->getDBO();
	...
	public function setCaption( $photoId , $caption )
	{
		$db		=& $this->getDBO();
