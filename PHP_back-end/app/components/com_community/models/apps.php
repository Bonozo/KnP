...
class CommunityModelApps extends JCCModel
{
	...
	public function addNeededCoreApps( $userId )
	{
		$db		= JFactory::getDBO();
		
		$query	= 'SELECT a.' . $db->nameQuote(EXTENSION_ID_COL_NAME).', a.' . $db->nameQuote('element') 
				.' FROM ' . $db->nameQuote(PLUGIN_TABLE_NAME) . ' AS a '
				.' LEFT JOIN ' . $db->nameQuote( '#__community_apps' ) . ' AS b '
				.' ON a.' . $db->nameQuote('element') .'=b.' . $db->nameQuote('apps')
				.' WHERE a.' . $db->nameQuote('folder') .'=' . $db->Quote( 'community' ) .' '
				.' AND b.' . $db->nameQuote('apps') .' is null';
