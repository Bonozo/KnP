...
implements CLimitsInterface, CNotificationsInterface
{
    ...
    public function CommunityModelFriends()
    {
         ...
         *@return boolean
         */
        public function addFriendGroup($id, $groupname)
        {
            $db = & $this->getDBO();
            $query = 'SELECT '. $db->nameQuote('group_name')
            		.' FROM '. $db->nameQuote('#__community_friendlist')
		        	.' WHERE '. $db->nameQuote('group_name').'='.$db->Quote($groupname)
            ...
            if ($name != $groupname)
            {
                $query = 'INSERT INTO '. $db->nameQuote('#__community_friendlist')
                		.' SET '. $db->nameQuote('group_name').'='.$db->Quote($groupname)
                		.','. $db->nameQuote('user_id').'='.$db->Quote($id);
                $db->setQuery($query);
                $db->query();
        ...
        public function setFriendsTag($vars)
        {
            $db = & $this->getDBO();
