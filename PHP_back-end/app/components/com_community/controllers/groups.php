...
class CommunityGroupsController extends CommunityBaseController
{
	...
	public function ajaxDeleteGroup( $groupId, $step=1 )
	{
		...
		if( !COwnerHelper::isCommunityAdmin() && !$isMine )
		{
...
<?php
	...
	public function ajaxApproveMember( $memberId , $groupId )
	{
		...
		else
		{
			...
			else
			{
				...
				CNotificationLibrary::add( 'groups.member.approved' , $group->ownerid , $user->id , JText::sprintf( 'COM_COMMUNITY_GROUP_MEMBER_APPROVED_EMAIL_SUBJECT' , $group->name ) , '' , 'groups.memberapproved' , $params );
	...
	public function ajaxRemoveMember( $memberId , $groupId )
	{
		...
		{
			$response->addScriptCall('joms.jQuery("#notice").html("' . JText::_('COM_COMMUNITY_GROUPS_MEMBERS_DELETE_DENIED') . '");');
			$response->addScriptCall('joms.jQuery("#notice").attr("class","error");');
		}
		else
		{
...
<?php
	...
	public function ajaxRemoveAdmin( $memberId , $groupId )
	{
		...
		else
		{
			...
			$response->addScriptCall('joms.jQuery("#member_' . $memberId . '").css("border","3px solid blue");');
			$response->addScriptCall('joms.jQuery("#notice").html("' . JText::_('COM_COMMUNITY_GROUPS_NEW_USER_MESSAGE') . '");');
			$response->addScriptCall('joms.jQuery("#notice").attr("class","info");');   
		}
	...
	public function ajaxAddAdmin( $memberId , $groupId )
	{
		...
		{       
			...
			$response->addScriptCall('joms.jQuery("#notice").html("' . JText::_('COM_COMMUNITY_GROUPS_NEW_ADMIN_MESSAGE') . '");');
			$response->addScriptCall('joms.jQuery("#notice").attr("class","info");');
		}
	...
	public function ajaxSaveDiscussionWall( $message , $uniqueId )
	{
		...
		CNotificationLibrary::add( 'groups.discussion.reply' , $my->id , $users , JText::sprintf( 'COM_COMMUNITY_GROUP_NEW_DISCUSSION_REPLY_SUBJECT' , $my->getDisplayName() , $discussion->title ) , '' , 'groups.discussion.reply' , $params );
	...
	public function ajaxSaveWall( $message , $groupId )
	{
		...
		else
		{
			...
			if( $groupParams->get( 'wallnotification' ) == '1' )
			{
				...
				$params->set('group' , $group->name );
				$params->set('message' , $message );
				CNotificationLibrary::add( 'groups.wall.create' , $my->id , $membersArray , JText::sprintf('COM_COMMUNITY_NEW_WALL_POST_NOTIFICATION_EMAIL_SUBJECT' , $my->getDisplayName() , $group->name ) , '' , 'groups.wall' , $params );
	...
	public function create()
	{
		...
		if( $config->get('creategroups')  &&  ( COwnerHelper::isCommunityAdmin() || (COwnerHelper::isRegisteredUser() && $my->canCreateGroups() ) ) )
		{
			...
			if(CLimitsHelper::exceededGroupCreation($my->id))
			{
				...
				echo JText::sprintf('COM_COMMUNITY_GROUPS_LIMIT' , $groupLimit);
				return;
			}
		...
		{
			echo JText::_('COM_COMMUNITY_GROUPS_DISABLE_CREATE_MESSAGE');
			return;
		}
	...
	public function search()
	{
		...
		if( $my->id == 0 && !$config->get('enableguestsearchgroups') )
		{
	...
	public function invitefriends()
	{
		...
		if( JRequest::getMethod() == 'POST' )
		{
			...
			if( !empty($invited ) )
			{
				...
				CNotificationLibrary::add( 'groups.invite' , $my->id , $invited , JText::sprintf('COM_COMMUNITY_GROUPS_JOIN_INVITATION_MESSAGE' , $group->name ) , '' , 'groups.invite' , $params );
	...
	public function addNews()
	{
		...
		{   
			...
			if( $validated )
			{
				...
				CNotificationLibrary::add( 'groups.create.news' , $my->id , $membersArray , JText::sprintf('COM_COMMUNITY_GROUPS_EMAIL_NEW_BULLETIN_SUBJECT' , $group->name ) , '' , 'groups.bulletin' , $params );
	...
	public function sendmail()
	{
		...
		if( JRequest::getMethod() == 'POST' )
		{
			...
			if( !$errors )
			{
				...
				$params->set( 'message' , $message );
				CNotificationLibrary::add( 'groups.sendmail' , $my->id , $emails , JText::sprintf( 'COM_COMMUNITY_GROUPS_SENDMAIL_SUBJECT' , $group->name ) , '' , 'groups.sendmail' , $params );
