...
class CommunityEventsController extends CommunityBaseController
{
	...
	public function ajaxAddAdmin( $memberId , $eventId )
	{
		...
		else
		{
			...
			$response->addScriptCall('joms.jQuery("#notice").html("' . JText::_('COM_COMMUNITY_EVENTS_ADD_ADMIN') . '");');
	...
	public function ajaxRemoveAdmin( $memberId , $eventId )
	{
		...
		else
		{
			...
			$response->addScriptCall('joms.jQuery("#member_' . $memberId . '").css("border","3px solid green");');
			$response->addScriptCall('joms.jQuery("#notice").html("' . JText::_('COM_COMMUNITY_EVENTS_REVERT_ADMIN') . '");');
			$response->addScriptCall('joms.jQuery("#notice").attr("class","info");');
		}
...
<?php
	...
	public function ajaxApproveInvite( $memberId , $eventId )
	{
		...
		else
		{
			...
			CNotificationLibrary::add( 'events.invitation.approved' , $event->creator , $user->id , JText::sprintf( 'COM_COMMUNITY_EVENTS_EMAIL_SUBJECT' , $event->title ) ,
	...
	public function sendmail()
	{
		...
		if( JRequest::getMethod() == 'POST' )
		{
			...
			if( !$errors )
			{
				...
				CNotificationLibrary::add( 'events.sendmail' , $my->id , $emails , JText::sprintf( 'COM_COMMUNITY_EVENT_SENDMAIL_SUBJECT' , $event->title ) , '' , 'events.sendmail' , $params );
	...
	public function invitefriends()
	{
		...
		if( JRequest::getMethod() == 'POST' )
		{
			...
			if( !empty($invited ) )
			{
				...
				CNotificationLibrary::add( 'events.invite' , $my->id , $invited ,JText::sprintf('COM_COMMUNITY_EVENTS_JOIN_INVITE' , $event->title ) , '' , 'events.invite' , $params );
