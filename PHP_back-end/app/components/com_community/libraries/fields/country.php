...
class CFieldsCountry
{
	...
	public function getFieldHTML( $field , $required )
	{
		...
		if( JFile::exists( $file ) )
		{
			...
			$tooltips		= !empty( $field->tips ) ? ' title="' . JText::_( $field->name ) . '::' . CStringHelper::escape( JText::_( $field->tips ) ) . '"' : '';
			CFactory::load( 'helpers' , 'string' );
			ob_start();
?>
			<select id="field<?php echo $field->id;?>" name="field<?php echo $field->id;?>" class="<?php echo !empty( $field->tips ) ? 'jomTips tipRight ' : '';?>select validate-country<?php echo $class;?> inputbox"<?php echo $tooltips;?>>
