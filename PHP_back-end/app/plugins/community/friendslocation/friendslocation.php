...
if(!class_exists('plgCommunityFriendsLocation'))
{
	class plgCommunityFriendsLocation extends CApplications
	{
		...
		function getFriends($userid, $limit)
		{
			...
			$query	= 'SELECT ' . $db->nameQuote( 'connect_from' ) . ' AS ids '
					. 'FROM ' . $db->nameQuote( '#__community_connection' ) . ' '
					. 'WHERE ' . $db->nameQuote( 'connect_to' ) . '=' . $db->Quote( $userid ) . ' '
		...
		function getFriendsLocation($friends, $town_field_id, $state_field_id, $country_field_id, $show_karma)
		{
			...
			$sql = 'SELECT 	a.'.$db->nameQuote('user_id').',
				      		a.'.$db->nameQuote('value').' AS country,
				      		b.'.$db->nameQuote('value').' AS state,
							c.'.$db->nameQuote('value').'	AS town'								      		
				    .' FROM '.$db->nameQuote('#__community_fields_values').' AS a' 
				    .' LEFT JOIN '.$db->nameQuote('#__community_fields_values').' AS b' 
				    .' ON a.'.$db->nameQuote('user_id').'=b.'.$db->nameQuote('user_id').' AND b.'.$db->nameQuote('field_id').' = '.$db->Quote($state_field_id)
				    .' LEFT JOIN '.$db->nameQuote('#__community_fields_values').' AS c'
				    .' ON a.'.$db->nameQuote('user_id').'=c.'.$db->nameQuote('user_id').' AND c.'.$db->nameQuote('field_id').' = '.$db->Quote($town_field_id)
				    .' WHERE a.'.$db->nameQuote('field_id').' = '.$db->Quote($country_field_id)
				    .' AND	a.'.$db->nameQuote('user_id').' IN ('.$friends_id.')';
		...
		{	
			...
			$css		= ( C_JOOMLA_15 ) 
						? JURI::base() . 'plugins/community/friendslocation/style.css' 
						: JURI::base() . 'plugins/community/friendslocation/friendslocation/style.css';
		...
		function _getFriendsLocationHTML($mapkey, $width, $height, $show_karma, $town_field_id, $state_field_id, $country_field_id, $userid, $def_limit, $layout)
		{
			ob_start();
