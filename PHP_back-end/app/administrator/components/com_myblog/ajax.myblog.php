...
function myxTogglePublishAdmin($id)
{
	...
	$objResponse = new JAXResponse();
	if ($publish){
		$objResponse->addAssign('pubImg' . $id, 'src', ((JVERSION >= 1.6) ? 'components/com_myblog/images/save_tag.gif' : 'images/publish_g.png') );
		...
		$objResponse->addAssign('pubImg' . $id, 'src', ((JVERSION >= 1.6) ? 'components/com_myblog/images/close_tag.gif' : 'images/publish_x.png'));
	}
	$objResponse->sendResponse();
	return;
}
...
/**
 *	Called via AJAX t oadd category.
 */ 
