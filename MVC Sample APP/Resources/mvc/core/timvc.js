...
var mainSpace = this;
//save main namespace for reference
...
var TiMVC = function(config) {
	...
	this.openWindow = function(window) {
		...
		if (self.iPhone) {
			...
			self.window.navGroup.open(window, {
				animated : true
			});
		} else {
	...
	this.closeWindow = function() {
		if (self.iPhone) {
			self.window.navGroup.close(self.window, {
				animated : false
			});
			return;
		}
	...
	this.openRootWindow = function() {
		...
		if (self.iPhone) {
			...
			self.window.navGroup = Titanium.UI.iPhone.createNavigationGroup({
				window : self.window
			});
	...
	this.includeBaseComponents = function() {
		Titanium.include(self.config.resDir + self.config.componentPath + "controller.js");
		//required!!
		Titanium.include(self.config.resDir + self.config.componentPath + "view.js");
		//required!!
		Titanium.include(self.config.resDir + self.config.componentPath + "database.js");
		//required!!
		Titanium.include(self.config.resDir + self.config.componentPath + "model.js");
		//required!!
	}
	/**
	 * Detect Platform
