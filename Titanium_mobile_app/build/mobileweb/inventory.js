...
var active_page = 1;
function renderInventoryPerPage(json,page_no){
	var counter;
	active_page = page_no;
	if(page_no == 1){
		counter = 0;
	...
	num_of_pages_in_inventory = parseInt(json.Record.length/9);
	if((json.Record.length % 9) != 0)
		num_of_pages_in_inventory ++;

	alert(num_of_pages_in_inventory );
	for(var i = 0; i < 3; i++){
		for(var j = 0; j < 3; j++){
			if(json.Record[counter] == undefined){
				rec = {IMAGE : ''};
				Ti.API.info('IMAGE: ');
			}
			else{
				Ti.API.info('IMAGE: '+json.Record[counter]);
				rec = json.Record[counter];
				//alert(rec);
			}
			...
			var gridItem1Icon = Titanium.UI.createImageView({
				image : getThumbImage(rec.IMAGE), 
				width : 50,// getThumbImageWidth(50),
				height : 50,//getThumbImageHeight(50)
			});
...
scrollUpButton.addEventListener('click', function() {
	if(active_page != 1){
		renderInventoryPerPage(json,--active_page);
	}
});
...
for(var i = 0; i < 3; i++){
	for(var j = 0; j < 3; j++){
		...
		//gridItem1Icon
		var gridItem1Icon = Titanium.UI.createImageView({
			image : getThumbImage(Ti.App.GLBL_items_image[counter]), 
			width : getThumbImageWidth(Ti.App.GLBL_items_image[counter]),
			height : getThumbImageHeight(Ti.App.GLBL_items_image[counter])
		});
		item1Button.add(gridItem1Icon);
