...
//Rare exceptions to indicate a bug in the _knightsandprincesses code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_knightsandprincesses = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_knightsandprincesses]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.knightsandprincesses.module"
