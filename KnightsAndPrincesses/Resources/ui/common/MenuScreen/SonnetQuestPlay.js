			...
			if (corr_res == '-1') {
				...
				if (counter == '3') {
					...
					if (!played) {
						...
						_url = "http://bonozo.com:8080/knp/knp_set_quest_status.php?game_status=LOSE&quest_status=" + quest_status + "&assign_quest_id=" + friend_quest_info.ASSIGN_QUEST_ID + "&quest_id=" + quest_id + "&uid=" + userinfo.Record[0].UID + "&friend_uid=" + friend_uid;
						httpclientt.requestServer({
							success : function(e) {
								items_json = JSON.parse(this.responseText);
								if (items_json.Record != undefined) {
