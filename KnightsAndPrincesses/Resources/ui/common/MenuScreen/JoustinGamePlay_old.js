function JoustinGamePlay(quest_status, quest_id, userinfo) {
	...
	function initknightSprite() {
		knight_image.scaleFromCenter(ScaleSpriteFromPercentage(winWidth, 196.25, 16), ScaleSpriteFromPercentage(winHeight, 256, 21), 0, 0);
		knight_image.x = getPixelFromPercent('x', 38);
		// - ((percent * 256 / 100) / 2);
		knight_image.y = getPixelFromPercent('y', 39.5);
		// - ((percent * 256 / 100) / 2);
	...
	window.addEventListener("open", function() {
		bg_image.scaleFromCenter(ScaleSpriteFromPercentage(winWidth, 400, 100), ScaleSpriteFromPercentage(winHeight, 198, 50), 0, 0);
		scene.add(bg_image);
		rails_image.scaleFromCenter(ScaleSpriteFromPercentage(winWidth, 200, 50), ScaleSpriteFromPercentage(winHeight, 193, 50), 0, 0);
