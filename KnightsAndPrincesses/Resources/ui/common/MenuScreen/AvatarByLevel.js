function AvatarByLevel(userinfo, tabledata, rowView, callback) {
	function getRandomInt(min, max) {
		return Math.floor(Math.random() * (max - min + 1)) + min;
	}

	var level_table_id = getRandomInt(1, 90000);
	var gender = '';
	...
	//var tabledata = [];
	var avatar_images = [];
	var AvatarLevelThumbnail = require('/ui/common/drawings/AvatarThumbnail');
	...
	httpclientt.requestServer({
		success : function(e) {
			...
			if (items_json.Record != undefined) {
				...
				//actInd.hide();
				callback(tableview);
				AvatarLevelThumbnail = null;
			}
		},
		method : 'GET',
		contentType : 'text/xml',
		url : "http://bonozo.com:8080/knp/get_avatar_list.php?uid=" + userinfo.Record[0].UID + "&gold=0"
	});
	Ti.App.addEventListener('avatar_table_changed', function(data) {
		if (data.release_table != 'AvatarByLevel')
			return;
