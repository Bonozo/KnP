function JoustGame(game, quest_id, userinfo) {
	if (game == 'joust_game') {
		...
		var _url = "http://justechinfo.com/kap_server/assign_quests.php?" + "assign_by_uid=" + userinfo.Record[0].UID + "&" + "assign_to_uid=" + userinfo.Record[0].UID + "&" + "quest_ids=" + quest_id + "&" + "message=Single Player Game";
		...
		gameImage.addEventListener('click', function() {
			if (!played) {
				_url = "http://justechinfo.com/kap_server/set_quests_status.php?" + "assign_quest_id=" + _assign_quest_id + "&status=COMPLETE";
				httpclientt.requestServer({
					success : function(e) {
						items_json = JSON.parse(this.responseText);
						if (items_json.Record != undefined) {
