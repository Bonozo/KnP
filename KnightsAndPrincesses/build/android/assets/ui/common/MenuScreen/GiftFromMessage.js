	...
	function sendMessage(Loggedin_id, friend_uid,text_feild, is_send) {
		if(is_send){
			...
			httpclientt.requestServer({
				...
				url : "http://bonozo.com:8080/knp/send_message.php?sender_id=" + Loggedin_id + "&receiver_id=" + friend_uid + "&message=" + text_feild + "",
					...
					for (var i = 0; i < (items_json.Record.length / view_per_row); i++) {
						...
						while (inventory_index < items_json.Record.length && j < 4) {
							...
							var imageview = Ti.UI.createImageView({
								image : '/assets/inv_128/' + items_json.Record[inventory_index].IMAGE + '.png',
								zIndex : 100,
						...
						ConfirmationAlert.addEventListener('click', function(e) {
							...
							switch (e.index) {
								...
								//alert(userinfo.Record[0].UID+":::"+selected_item.id);
									...
									//actInd.show();
									var send_gift_url = "http://bonozo.com:8080/knp/send_gift.php?sender_id=" + userinfo.Record[0].UID + "&receiver_id=" + freind_uid + "&gift_id=" + selected_item.id + "&gift_type=" + selected_item.type;
									var httpclientt = require('/ui/common/Functions/function');
